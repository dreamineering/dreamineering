...
// type BlockProps = SVGProps<SVGPathElement> & BlockSpecificProps;
type BlockProps = MotionProps & BlockSpecificProps;

const Block: React.FC<BlockProps> = ({ x, y, ...props }) => {
  return (
    <motion.path
...
export const GridPattern: React.FC<GridPatternProps> = ({
  ...
  let currentBlock = useRef<number[] | null>(null);
      ...
      setHoveredBlocks((blocks) => {
        ...
        return [...blocks, [x, y, key] as [number, number, number]].filter(
          (block: [number, number, number]) =>
            !(block[0] === x && block[1] === y && block[2] !== key)
        );
      });
    }
